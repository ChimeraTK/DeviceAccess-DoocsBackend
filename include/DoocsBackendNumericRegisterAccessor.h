...
namespace ChimeraTK {
  ...
  : DoocsBackendRegisterAccessor<UserType>(
    ...
    try {
      // callForDoocsType() will throw a runtime_error if the type is not supported, except for DATA_IFFF which we have
      // to catch first
      if(this->src.type() != DATA_IFFF) {
    ...
    switch(this->dst.type()) {
      ...
      case DATA_IFFF: {
        for(size_t i = 0; i < this->nElements; i++) {
          if(i + this->elementOffset == 0) {
            this->buffer_2D[0][i] = ChimeraTK::numericToUserType<UserType>(dataGet<int>(i + this->elementOffset));
          }
          else {
            this->buffer_2D[0][i] = ChimeraTK::numericToUserType<UserType>(dataGet<float>(i + this->elementOffset));
          }
        }
        return;
      }
